...
export default function StudioPage() {
  ...
  const [counts,setCounts]=useState([20,10,10,20,10,10,5]);
 ...
 console.log("Studio locale:",
  ...
  <div className={styles.welcomeRow}>
    ...
    <div ref={languageRef} className={styles.languageBox}>
      ...
      {showLanguage && (
        <div className={styles.languageMenu}>
          ...
          <button onClick={() => { setLocale("en"); setShowLanguage(false); }}>
            🇺🇸 English
          </button>

          <button onClick={() => { setLocale("fa"); setShowLanguage(false); }}>
            🇮🇷 فارسی
          </button>

          <button onClick={() => { setLocale("tr"); setShowLanguage(false); }}>
            🇹🇷 Türkçe
          </button>

          <button onClick={() => setShowLanguage(false)}>🇦🇲 Հայերեն</button>
          <button onClick={() => setShowLanguage(false)}>🇵🇰 اردو</button>
          <button onClick={() => setShowLanguage(false)}>🇸🇦 العربية</button>
          <button onClick={() => setShowLanguage(false)}>🇦🇿 Azərbaycan</button>
          <button onClick={() => setShowLanguage(false)}>🇰🇿 Қазақша</button>
          <button onClick={() => setShowLanguage(false)}> Kurdî</button>
          <button onClick={() => setShowLanguage(false)}>🇧🇩 বাংলা</button>
